...
$builder = Join-Path $repoRoot "scripts\build_ceo_daily_pnl_package_v4_6_1.py"
$validator = Join-Path $repoRoot "scripts\validate_ceo_daily_pnl_package_v4_6_1.py"
...
Write-Host "Building CEO Daily P&L v4.6.1 for $ReportDate ..."
& python @arguments
if ($LASTEXITCODE -ne 0) { throw "v4.6.1 builder failed with exit code $LASTEXITCODE" }

$package = Join-Path $outDir ("ceo_daily_pnl_package_v4_6_1_{0}.json" -f $ReportDate)
...
if ($LASTEXITCODE -ne 0) { throw "v4.6.1 validation failed" }
