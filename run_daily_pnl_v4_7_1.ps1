...
$builder = Join-Path $repoRoot "scripts\build_ceo_daily_pnl_package_v4_7_1.py"
$validator = Join-Path $repoRoot "scripts\validate_ceo_daily_pnl_package_v4_7_1.py"
...
Write-Host "Building CEO Daily P&L v4.7.1 (Top20 + Inventory Sales Mapping) for $ReportDate ..."
& python @arguments
if ($LASTEXITCODE -ne 0) { throw "v4.7.1 builder failed with exit code $LASTEXITCODE" }

$package = Join-Path $outDir ("ceo_daily_pnl_package_v4_7_1_{0}.json" -f $ReportDate)
...
if ($LASTEXITCODE -ne 0) { throw "v4.7.1 validation failed" }

Write-Host "PASS: $package"
Write-Host "Top SKU: Top 20 per channel by MTD sold qty, gifts excluded from rank."
Write-Host "Inventory sheet: one row per SKU, stock summed across expiry lots, with NHANH/SHOPEE/TIKTOK sold qty."
