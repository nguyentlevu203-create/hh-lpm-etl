...
# Keep the growth v3.3 builder and v4.6 on the same DB environment without printing secrets.
...
Write-Host "Building CEO Daily P&L v4.6 for $ReportDate ..."
& python @arguments
if ($LASTEXITCODE -ne 0) { throw "v4.6 builder failed with exit code $LASTEXITCODE" }

$package = Join-Path $outDir ("ceo_daily_pnl_package_v4_6_{0}.json" -f $ReportDate)
...
if ($LASTEXITCODE -ne 0) { throw "v4.6 validation failed" }
